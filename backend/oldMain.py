...
POST /api/sessions/acoustic-emotion — Pipeline A: WAV chunk → librosa → emotion probs
                                      (called every ~100ms by frontend for live waveform)
"""
...
# Loaded lazily on first /detect-emotion call so startup is not blocked.
...
# Exact port of desktop app _pipeline_acoustic().
# Used both for /voice-message (full clip) and /acoustic-emotion (live chunk).
try:
    import librosa
    import numpy as np
    LIBROSA_OK = True
...
# Emotion constants + HARTMANN_MAP
...
def _pipeline_acoustic(audio_np: "np.ndarray") -> dict:
    """
    Exact port of desktop app _pipeline_acoustic().
    Takes float32 numpy array of 16 kHz mono audio.
    Returns prob dict over EMOTION_KEYS_PY.
    """
    import numpy as np
    import librosa
    ...
    # ── Exact librosa calls from desktop app ──────────────────────────────────
    ...
    # ── Exact raw score formula from desktop app ──────────────────────────────
    if rms < 0.001:
    ...
    print(f"[Pipeline A] rms={rms:.4f} zcr={zcr:.3f} cent={int(cent)}Hz "
          f"roll={int(roll)}Hz bw={int(bw)}Hz mfcc={mfcc:.2f} "
...
def _pipeline_semantic_text(text: str) -> dict:
    """
    Exact port of desktop app _pipeline_semantic() NLP section.
    Takes already-transcribed text, runs Hartmann, returns prob dict.
    """
    nlp = _load_nlp()
    if nlp is None:
        v = 1.0 / len(EMOTION_KEYS_PY)
    ...
    preds = nlp(text[:512])[0]          # list of {label, score}
    ...
    print(f"[Pipeline B] \"{text[:60]}\" → {max(sp, key=sp.get).upper()} "
          f"({sp[max(sp, key=sp.get)]:.0%})")
    return sp


def _fuse(acoustic_probs: dict, semantic_probs: dict, has_speech: bool) -> tuple:
    """
    Exact port of desktop app _fuse().
    Returns (emotion_str, fused_prob_dict).
    """
    if has_speech:
...
class AcousticChunkReq(BaseModel):
    """
    Frontend sends a small WAV chunk (base64) every ~100 ms.
    Backend runs librosa Pipeline A and returns the emotion probs.
    No auth required — this is a hot path called very frequently.
    """
    audio_b64: str
...
# ═══════════════════════════════════════════════════════════════════════════════
THERAPIST_SYSTEM = (
    "You are Serene, a warm and licensed psychological therapist. "
    "Respond in 3–5 calm, empathetic sentences. "
    "Do NOT give medical diagnoses. "
    "End with one gentle, open-ended follow-up question. "
    "Sound natural and human. Output only the therapist response — no labels or preamble."
)
...
NEW_USER_GREETING_PROMPT = [
    {"role": "system", "content": THERAPIST_SYSTEM},
    {
        "role": "user",
        "content": (
            "A new client is starting their very first therapy session. "
            "Welcome them warmly, let them know this is a safe space, "
            "and gently invite them to share what brought them here today. "
            "Do not mention memory or past sessions."
        ),
    },
]
...
def returning_user_greeting_prompt(past_summary: str) -> list:
    return [
        {"role": "system", "content": THERAPIST_SYSTEM},
        {
            "role": "system",
            "content": (
                "This client has visited before. Use the following session summary "
                "to provide continuity — do NOT quote it directly or mention 'last session summary'. "
                "Simply let your greeting reflect that you remember them and their journey:\n\n"
                + past_summary
            ),
        },
        {
            "role": "user",
            "content": (
                "The client has returned. Greet them warmly as someone you know. "
                "Acknowledge their return gently, and invite them to share how they have been."
            ),
        },
    ]
...
def _build_conversation_messages(session: dict, db_history: list, emotion: str) -> list:
    ...
    if past:
        msgs.append({
            "role": "system",
            "content": (
                "Prior session context (do NOT reference directly — use it only "
    ...
    if emotion and emotion != "neutral":
        msgs.append({
            "role": "system",
            "content": (
                f"[Internal note: the client's detected emotion is '{emotion}'. "
                "Acknowledge it naturally if appropriate — do not mention this note.]"
            ),
        })
...
def _summarise(transcript: str) -> str:
    messages = [
        {"role": "system", "content": (
            "You are a therapist's assistant. Write a concise 4–6 sentence summary of this "
            "session covering: main themes, client emotional arc, key insights, unresolved concerns. "
            "This summary seeds the next session's memory."
        )},
...
def start_session(user=Depends(get_user)):
    ...
    if has_past_memory:
        greeting_msgs = returning_user_greeting_prompt(past_summary)
    else:
        greeting_msgs = NEW_USER_GREETING_PROMPT
    ...
    sessions_col.insert_one({
        ...
        "status": "active",
        "started_at": now,
        "has_past_memory": has_past_memory,
...
def voice_message(body: VoiceReq, user=Depends(get_user)):
    ...
    # ── Pipeline B — Hartmann NLP on Whisper transcript ───────────────────────
    ...
    # ── Fusion — 40% acoustic + 60% semantic ─────────────────────────────────
    has_speech     = bool(transcript.strip())
    detected_emotion, fused_probs = _fuse(acoustic_probs, semantic_probs, has_speech)
    print(f"[Fusion] acoustic={max(acoustic_probs,key=acoustic_probs.get).upper()} "
          f"semantic={max(semantic_probs,key=semantic_probs.get).upper()} "
          f"→ FUSED={detected_emotion.upper()}")
...
def detect_emotion(body: EmotionTextReq):
    """
    Pipeline B — Hartmann NLP on text.
    Called by frontend every 2 s with the rolling Web Speech API transcript.
    """
    ...
    try:
        ...
        s_emotion = max(sp, key=sp.get)
        return {"emotion": s_emotion, "probs": sp}
    except Exception as e:
...
def acoustic_emotion(body: AcousticChunkReq):
    """
    Pipeline A — librosa acoustic features on a raw PCM chunk.

    Called by the frontend every ~100 ms during live recording.
    Accepts a base64-encoded WAV (16 kHz mono, ~2048 samples) and returns
    the exact same librosa-based prob dict that the desktop app computes,
    replacing the frontend's naive JS DFT approximation entirely.

    No auth header required — this is a high-frequency hot path.
    If librosa is unavailable the endpoint gracefully returns flat probs.
    """
    if not LIBROSA_OK:
        flat = 1.0 / len(EMOTION_KEYS_PY)
        return {"emotion": "neutral", "probs": {k: flat for k in EMOTION_KEYS_PY}}
    ...
    try:
        ...
        # Load directly from bytes — no temp file needed for small chunks
