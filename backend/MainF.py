...
TURN_GOALS = {
    1: (
        "Your only job this turn: make them feel safe and heard. "
        "Ask one gentle open question — what's been weighing on them lately? "
        "Do NOT ask anything else. 2 sentences max."
    ),
    2: (
        "They just opened up. Reflect one specific thing they said — use their exact word(s). "
        "Then ask: how long has this been going on? Is it new or has it been building? "
        "One question. 2-3 sentences."
    ),
    3: (
        "You know the situation and roughly how long. Acknowledge the timeline with warmth. "
        "Then ask how this is hitting their day-to-day — sleep, focus, body, or relationships — "
        "pick whichever angle their previous answers hint at most. One question. 2-3 sentences."
    ),
    4: (
        "You know the daily impact. Reflect it back briefly using their words. "
        "Then ask: is there anyone around them who knows — family, friend, anyone? "
        "Do they feel alone in this? One question. 2-3 sentences."
    ),
    5: (
        "You have their support picture now. Acknowledge whether they're carrying this alone or not. "
        "Then ask: have they tried anything to cope — anything at all, even small things? "
        "What helped even a little, what made it worse? One question. 2-3 sentences."
    ),
    6: (
        "You know what they've tried. Reflect that briefly — especially anything that helped. "
        "Then ask: when this hits hardest, what does that moment actually feel like? "
        "Help them name it — physically, emotionally. One question. 2-3 sentences."
    ),
    7: (
        "Acknowledge the weight of what they described. Don't minimise it. "
        "Then ask: what does a good day look like for them right now — even a small one? "
        "What's different about those days? One question. 2-3 sentences."
    ),
    8: (
        "Hold both pictures — worst and best moments. Let them feel you see the full range. "
        "Then ask: what do they think is underneath all of this — what's the real root? "
        "Trust their instinct. One question. 2-3 sentences."
    ),
    9: (
        "They've named the root. Validate it honestly — not with empty praise. "
        "Then ask: what would a small, real step forward look like this week — even tiny? "
        "This is the last question before the remedy. One question. 2-3 sentences."
    ),
    10: (
        "THIS IS THE REMEDY TURN. No questions. Flowing natural conversation only. "
        "Structure (invisible to reader):\n"
        "• 1-2 sentences: reflect the whole story using their exact words from the conversation.\n"
        "• 1 sentence: name the real pattern underneath it — honestly, not clinically.\n"
        "• 3 specific practices tied directly to what they described — weave them as natural suggestions, "
        "  not a list. Each one must reference something they actually said. Small enough to do this week.\n"
        "• Tell them which to start with tonight, and why — based on what they said in turn 9.\n"
        "• End with one warm, human, friend-like send-off. Not scripted. Not clinical.\n"
        "Total: 5-7 sentences. No bullet points. No headers. Pure conversation."
    ),
}

# ── Master system prompt ──────────────────────────────────────────────────────
SYSTEM_PROMPT = """You are Serene — a warm, present companion who happens to understand how the mind works. You are NOT a chatbot, NOT a script. You are having one real conversation with one real person.

ABSOLUTE OUTPUT RULES:
1. MAXIMUM 100 WORDS per response. Count them. Stay under.
2. Output ONLY your spoken words. No labels, no "Serene:", no turn numbers, no headers, no bullet points, no asterisks.
3. Pure flowing sentences. Nothing else.
4. Contractions. Natural rhythm. Short sentences mixed with longer ones.
5. Sound like a person — warm, real, direct.

BANNED WORDS (never use): validate, reframe, unpack, hold space, sit with, efficacy, recalibration, journey, empower, processing, closure, narrative, boundaries (as therapy concept), trauma-informed, self-care (unless they said it first), mindfulness (unless they said it first).

THREADING RULE: Every response must reference at least one specific thing the person said — use their exact word or phrase. Connect this moment to something earlier. This is one continuous story.

TONE: Match their energy. Slow and gentle if they're heavy. Steady if they're scared. Direct if they're frustrated. Never perform warmth — let it live in your words.

ONE QUESTION RULE (turns 1-9): Ask exactly ONE question per response. Never two. The question comes at the end, woven naturally into the sentence.

TURN 10: No questions. Pure remedy. Still under 100 words. Still human."""


def _build_messages(session: dict, db_history: list, profile: dict, emotion: str, turn_num: int) -> list:
    """Build the full prompt chain for this specific turn."""

    msgs = [{"role": "system", "content": SYSTEM_PROMPT}]

    # ── Profile context (what we know about this person across sessions) ──────
    profile_lines = []
    if profile.get("recurring_themes"):
        profile_lines.append("Recurring themes from past sessions: " + ", ".join(profile["recurring_themes"]))
    if profile.get("past_remedies"):
        profile_lines.append("Remedies tried before: " + "; ".join(profile["past_remedies"][-3:]))
    if profile.get("life_context"):
        ctx = profile["life_context"]
        bits = [f"{k}: {v}" for k, v in ctx.items() if v]
        if bits:
            profile_lines.append("Known about their life: " + ", ".join(bits))

    if profile_lines:
        msgs.append({
            "role": "system",
            "content": (
                "WHAT YOU ALREADY KNOW ABOUT THIS PERSON (from past sessions — "
                "use for warmth and continuity, never reference directly):\n"
                + "\n".join(profile_lines)
            )
        })
    ...
    if past:
        msgs.append({
            "role": "system",
            "content": (
                "Memory from their last session (let this inform your intuitions — "
                "do NOT say 'last time' or 'your summary'):\n" + past
            )
        })

    # ── Cumulative story so far (numbered by turn) ────────────────────────────
    user_turns = [(i+1, h["content"]) for i, h in enumerate(db_history) if h.get("role") == "user"]
    if user_turns:
        lines = ["WHAT THIS PERSON HAS SAID SO FAR — use their exact words in your response:"]
        for n, content in user_turns:
            lines.append(f"Turn {n}: {content[:500]}" + ("…" if len(content) > 500 else ""))
        msgs.append({"role": "system", "content": "\n".join(lines)})

    # ── Per-turn goal ─────────────────────────────────────────────────────────
    goal = TURN_GOALS.get(turn_num, TURN_GOALS[10])
    msgs.append({
        "role": "system",
        "content": (
            f"YOUR GOAL FOR THIS RESPONSE (turn {turn_num} of 10):\n{goal}\n\n"
            "BEFORE YOU OUTPUT — check:\n"
            "[ ] Under 100 words?\n"
            "[ ] Using at least one of their exact words?\n"
            "[ ] Exactly 1 question (turns 1-9) or 0 questions (turn 10)?\n"
            "[ ] No labels, headers, or bullet points?\n"
    ...
    # ── Emotion tone note (internal, never surfaced) ──────────────────────────
    tone_map = {
        "sad":     "They sound sad or low. Be slow, close, gentle. Don't rush.",
        "anger":   "They sound frustrated or angry. Acknowledge it directly first.",
        "excited": "They sound anxious or activated. Be steady. Help them slow down.",
        "happy":   "They sound okay. Lighter tone is fine but still go deep.",
        "calm":    "They sound calm and reflective. Go deeper and more exploratory.",
    }
    tone = tone_map.get(emotion, "")
    if tone:
        msgs.append({"role": "system", "content": f"[Internal tone note — never mention]: {tone}"})
...
def _llm(messages: list) -> str:
    try:
        ...
        # Hard-trim to ~100 words as a safety net
        words = text.split()
        if len(words) > 110:
            text = " ".join(words[:100])
...
def _handle_user_message(session_id: str, user: dict, text: str, emotion: str) -> dict:
    ...
    # Determine turn number (capped at 10)
    turn_num = min(user_turn_count, 10)
